...
const Appointment = () => {
  ...
  const getAvailableSolts = async () => {
    ...
    const sessions = [
        ...
        { start: 14, end: 20 }, // 2:00 PM to 8:00 PM
        { start: 21, end: 23 }  // 9:00 PM to 11:00 PM
    ];
    ...
    for (let i = 0; i < 7; i++) {
        ...
        for (const session of sessions) {
            let sessionStart = new Date(day);
            let sessionEnd = new Date(day);

            sessionStart.setHours(session.start, 0, 0, 0);
            sessionEnd.setHours(session.end, 0, 0, 0);

            // For today, skip past slots
            if (i === 0 && now > sessionEnd) continue;
            if (i === 0 && now > sessionStart) {
                sessionStart = new Date(now);

                // Round up to next half hour
                const minutes = sessionStart.getMinutes();
                if (minutes <= 30) {
                    sessionStart.setMinutes(30, 0, 0);
                } else {
                    sessionStart.setHours(sessionStart.getHours() + 1, 0, 0, 0);
                }
                if (sessionStart >= sessionEnd) continue;
            }

            while (sessionStart < sessionEnd) {
                const formattedTime = sessionStart.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                const slotDate = `${sessionStart.getDate()}_${sessionStart.getMonth() + 1}_${sessionStart.getFullYear()}`;
                ...
                if (isSlotAvailable) {
                    slots.push({
                        datetime: new Date(sessionStart),
                        time: formattedTime,
                    });
                }
